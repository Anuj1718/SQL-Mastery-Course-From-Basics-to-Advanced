...
WITH states as ( -- saving this result in a virtual table named states
	SELECT state, SUM(shipping_cost) AS sum_shipping_cost
	FROM Sales_Dataset
	GROUP BY state
	ORDER BY sum_shipping_cost DESC
	LIMIT 3
)SELECT * FROM states -- i wanna display the all results in states now
-- after you complete bracket, only then you can use it in that session
select * from states; -- error
